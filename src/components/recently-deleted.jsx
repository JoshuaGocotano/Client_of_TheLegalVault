...
export default function RecentlyDeleted({ onClose }) {
    ...
    const fetchAllDeletedItems = async () => {
        ...
        try {
            ...
            if (paymentsResponse.ok) {
                ...
                const paysArr = Array.isArray(paymentsData)
                    ? paymentsData
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 dark:from-gray-900 dark:via-gray-950 dark:to-blue-950 flex flex-col">
            {/* Header */}
                ...
                <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
                    <div className="flex h-16 items-center justify-between">
                        <div className="flex items-center gap-3">
                            <button
                                onClick={onClose}
                                className="rounded p-2 hover:bg-blue-50 dark:hover:bg-gray-800 focus:outline-none transition-colors"
                                title="Back"
                            >
                            </button>
                        ...
                        <div className="relative w-64">
                            ...
                            <input
                                ...
                                className="w-full rounded-lg border border-gray-200 bg-white py-1.5 pl-9 pr-3 text-gray-900 placeholder-gray-400 focus:border-blue-400 focus:ring-1 focus:ring-blue-200 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100 dark:placeholder-gray-500 shadow-sm"
                            />
                        </div>
                    </div>
                </div>
            ...
            <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 lg:px-8 w-full flex-1">
                {/* Tabs */}
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex items-center gap-2 px-2 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.id
                            ...
                            >
                                ...
                                {tab.label}
                                {tab.count > 0 && (
                                    <span className="ml-1 rounded bg-blue-50 px-2 py-0.5 text-xs text-blue-700 dark:bg-gray-800 dark:text-blue-200">{tab.count}</span>
                                )}
                            </button>
                        );
                    })}
                ...
                <div className="flex items-center gap-2 mb-4 mt-2">
                    ...
                    <span className="text-lg font-semibold text-gray-700 dark:text-gray-200 capitalize">{activeTab}</span>
                    ...
                    {loading ? (
                        ...
                        <>
                            <div className="grid gap-4">
                                {activeTab === "documents" && getPaginatedData().map((doc, idx, arr) => (
                                    <React.Fragment key={doc.id}>
                                        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-md hover:shadow-lg transition-shadow flex items-center justify-between dark:border-gray-700 dark:bg-gray-800 animate-fadeIn">
                                            <div className="flex items-center gap-3">
                                                <File className="h-8 w-8 text-blue-400" />
                                                <div>
                                                    <div className="font-semibold text-gray-900 dark:text-gray-100">{doc.doc_name}</div>
                                                    <div className="text-xs text-gray-500 dark:text-gray-400">{doc.doc_type} • {doc.case_name}</div>
                                                    <div className="text-xs text-gray-400">Deleted by {doc.deleted_by} on {formatDate(doc.deleted_date)}</div>
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-end gap-2 min-w-[120px]">
                                                <span className="mb-1 inline-block rounded-full bg-yellow-100 px-2 py-0.5 text-xs text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200" title="Days left before permanent deletion">
                                                    {getDaysLeft(doc.deleted_date)} days left
                                                </span>
                                                <div className="flex gap-2">
                                                    <button onClick={() => handleRestore(doc.id, "documents")}
                                                        className="rounded bg-green-500 px-3 py-1.5 text-xs text-white hover:bg-green-600 focus:ring-2 focus:ring-green-300 focus:outline-none shadow transition-all duration-150"
                                                        title="Restore this document">Restore</button>
                                                    <button onClick={() => handlePermanentDelete(doc.id, "documents")}
                                                        className="rounded bg-red-500 px-3 py-1.5 text-xs text-white hover:bg-red-600 focus:ring-2 focus:ring-red-300 focus:outline-none shadow transition-all duration-150"
                                                        title="Permanently delete this document">Delete Permanently</button>
                                                </div>
                                            </div>
                                        </div>
                                        {idx < arr.length - 1 && <div className="border-t border-gray-100 dark:border-gray-800 mx-2" />}
                                    </React.Fragment>
                                ))}
                                {activeTab === "tasks" && getPaginatedData().map((task, idx, arr) => (
                                        ...
                                        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-md hover:shadow-lg transition-shadow flex items-center justify-between dark:border-gray-700 dark:bg-gray-800 animate-fadeIn">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="text-xs text-gray-500 dark:text-gray-400">{task.assigned_to} • {task.case_name}</div>
                                                    <div className="text-xs text-gray-400">Deleted by {task.deleted_by} on {formatDate(task.deleted_date)}</div>
                                                </div>
                                            </div>
                                            <div className="flex flex-col items-end gap-2 min-w-[120px]">
                                                <span className="mb-1 inline-block rounded-full bg-yellow-100 px-2 py-0.5 text-xs text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200" title="Days left before permanent deletion">
                                                ...
                                                <div className="flex gap-2">
                                                    <button onClick={() => handleRestore(task.id, "tasks")}
                                                        className="rounded bg-green-500 px-3 py-1.5 text-xs text-white hover:bg-green-600 focus:ring-2 focus:ring-green-300 focus:outline-none shadow transition-all duration-150"
                                                        title="Restore this task">Restore</button>
                                                    <button onClick={() => handlePermanentDelete(task.id, "tasks")}
                                                        className="rounded bg-red-500 px-3 py-1.5 text-xs text-white hover:bg-red-600 focus:ring-2 focus:ring-red-300 focus:outline-none shadow transition-all duration-150"
                                                        title="Permanently delete this task">Delete Permanently</button>
                                                </div>
                                            </div>
                                        </div>
                                        {idx < arr.length - 1 && <div className="border-t border-gray-100 dark:border-gray-800 mx-2" />}
